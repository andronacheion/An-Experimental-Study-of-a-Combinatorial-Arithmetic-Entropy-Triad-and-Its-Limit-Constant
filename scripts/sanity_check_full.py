...
from src.tails import triad_tail_bound  # must return (R_Xi, R_E, R_Omega, R_total)
...
def compute_values(points, N_ref: int, D_omega: float, progress: bool):
    ...
    Returnează:
      ...
      kappa_interval: (kappa_min, kappa_max) din F(N_ref) ± R_total(N_ref)
    """
...
def main():
    ...
    header = (
        f"{'n':>10}  {'F(n)':>14}  "
        f"{'R_Xi':>9}  {'R_E':>9}  {'R_Omega':>9}  {'R_tot':>9}  "
        f"{'|F-k_mid|':>11}  {'OK?':>4}  {'scaled rem.':>12}  {'ΔF vs REF':>12}"
    )
    print("\n" + header)
    print("-" * len(header))
    ...
    for n in points:
        ...
        r_xi, r_e, r_om, r_tot = R[n]
        # Certificat: |F(n) - kappa_mid| ≤ R_tot(n) + R_tot(N_ref)
        ok = abs(f_val - kappa_mid) <= (r_tot + (kappa_max - kappa_min) / 2.0)
        scaled = (n * (f_val - kappa_mid) / log(n)) if n > 1 else float('nan')
        ref = REF_F.get(n, float('nan'))
        delta = f_val - ref if ref == ref else float('nan')
        print(
            f"{n:10d}  {f_val:14.10f}  "
            f"{r_xi:9.2e}  {r_e:9.2e}  {r_om:9.2e}  {r_tot:9.2e}  "
            f"{abs(f_val-kappa_mid):11.3e}  "
            f"{'OK' if ok else 'FAIL':>4}  {scaled:12.6f}  "
            f"{(delta if ref == ref else float('nan')):12.2e}"
        )

    print("\nNote:")
    print("• OK înseamnă că |F(n) - kappa_mid| ≤ R_tot(n) + R_tot(N_ref) (compatibil cu certificatul).")
    print("• scaled remainder = n (F(n) - kappa_mid) / ln n; aștepți ≈ 1/ln 2 ≈ {:.6f} pentru n mare.".format(one_over_ln2))
    print("• R_Xi, R_E, R_Omega sunt componentele certificate ale cozii; R_tot = suma lor.")
